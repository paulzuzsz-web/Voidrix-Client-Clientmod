...
import gg.voidrix.compat.auth.DevPinStore
...
import gg.voidrix.compat.auth.DevPinStore.Role
import gg.voidrix.compat.auth.launcher.AuthHandback
import gg.voidrix.compat.auth.launcher.Credentials
import gg.voidrix.compat.auth.launcher.LocalRedirectServer
...
import gg.voidrix.compat.auth.launcher.DeviceCodeFlow.Challenge
import gg.voidrix.compat.auth.launcher.LoginOrchestrator.LoginStep
import gg.voidrix.compat.client.MCLogger
import gg.voidrix.compat.input.ClipboardHelper
...
import gg.voidrix.owolib.owo.ui.core.UIComponent.DismountReason
...
public class AccountSelectContent(onClose: () -> Unit) : FlowLayout(Sizing.Companion.fill(100), Sizing.Companion.content(), Algorithm.VERTICAL) {
   ...
   private final var addState: gg.voidrix.client.v2.auth.AccountSelectContent.AddState
   ...
   private fun rebuild() {
      ...
      if (this.addState is AccountSelectContent.AddState.Active) {
         ...
         if (all.isEmpty()) {
            ...
            val var10002: MutableComponent = Component.translatable("voidrix.auth.select.empty", Arrays.copyOf(acc, acc.length))
   ...
   private fun renderFooter(all: List<Credentials>) {
      ...
      if (all.size() == 1) {
         var10000 = this.t("voidrix.auth.select.count.one")
      } else {
         val var14: Array<Any> = arrayOf(all.size())
         val var29: MutableComponent = Component.translatable("voidrix.auth.select.count.many", Arrays.copyOf(var14, var14.length))
      ...
      val var20: V3Button = V3Button(
         TextKt.toSmallCaps(this.t("voidrix.auth.select.button.refresh")), { var1: VoidrixLabelButton, var2: Double, var4: Double, var6: Int ->
      ...
      val var21: V3Button = V3Button(TextKt.toSmallCaps(this.t("voidrix.auth.select.button.add")), { var1: VoidrixLabelButton, var2: Double, var4: Double, var6: Int ->
         UISounds.playButtonSound()
   ...
   private fun renderRolePicker(role: Role) {
      ...
      if ((var32 as java.util.List).isEmpty()) {
         ...
         val var10002: MutableComponent = Component.translatable("voidrix.auth.select.empty", Arrays.copyOf(var24, var24.length))
   ...
   private fun rowSubText(acc: Credentials, isRefreshingThis: Boolean, expired: Boolean): MutableComponent {
      ...
      if (isRefreshingThis) {
         ...
         var10000 = Component.translatable("voidrix.auth.select.state.refreshing", Arrays.copyOf(`args$iv`, `args$iv`.length))
      } else if (expired) {
         val var9: Array<Any> = arrayOfNulls(0)
         var10000 = Component.translatable("voidrix.auth.select.sub.expired", Arrays.copyOf(var9, var9.length))
      } else {
         val var10: Array<Any> = arrayOfNulls(0)
         var10000 = Component.translatable("voidrix.auth.select.sub.signedIn", Arrays.copyOf(var10, var10.length))
      }
      ...
      return if (isRefreshingThis)
         ...
         (
            if (acc.getActive() && expired)
               this.badgeOf(this.t("voidrix.auth.select.badge.expired"), V3Theme.INSTANCE.gray(12), V3Theme.INSTANCE.getError() and 16777215 or 1610612736)
               else
               (
                  if (acc.getActive())
                     this.badgeOf(this.t("voidrix.auth.select.badge.active"), V3Theme.INSTANCE.accentAlpha(11, 220), V3Theme.INSTANCE.accentAlpha(4, 80))
                     else
                     null
               )
         )
   ...
   private fun renderDeviceCodePane() {
      ...
      if ((this.addState as? AccountSelectContent.AddState.Active) != null) {
         ...
         if (var10000.phase != null) {
            ...
            val var10002: MutableComponent = Component.translatable("voidrix.auth.select.devicecode.requesting", Arrays.copyOf(var20, var20.length))
   ...
   private fun buildPhaseBox(phase: String): FlowLayout {
      ...
      var2.gap(6)
      var var5: LabelComponent = LabelComponent(TextKt.getLiteral(TextKt.toSmallCaps(this.t("voidrix.auth.select.devicecode.title"))) as Component)
      this.liveColor(var5, { 
         V3Theme.INSTANCE.grayColor(12)
      })
   ...
   private fun buildChallengeBody(challenge: Challenge, codeCopied: Boolean): FlowLayout {
      ...
      var3.gap(6)
      val var6: LabelComponent = LabelComponent(TextKt.getLiteral(TextKt.toSmallCaps(this.t("voidrix.auth.select.devicecode.title"))) as Component)
      this.liveColor(var6, { 
         V3Theme.INSTANCE.grayColor(12)
      })
      var6.scale(1.05F)
      ...
      for (var21 in StringsKt.split$default(this.t("voidrix.auth.select.devicecode.blurb"), arrayOf("\n"), false, 0, 6, null)) {
      ...
      val var34: V3Button = V3Button(
         TextKt.toSmallCaps(this.t(if (codeCopied) "voidrix.auth.select.devicecode.copied" else "voidrix.auth.select.devicecode.copy")),
         { var2: VoidrixLabelButton, var3: Double, var5: Double, var7: Int ->
            UISounds.playButtonSound()
      ...
      val var10003: MutableComponent = Component.translatable("voidrix.auth.select.devicecode.waiting", Arrays.copyOf(var41, var41.length))
   ...
   private fun buildReopenLink(challenge: Challenge): LabelComponent {
      val `args$iv`: Array<Any> = arrayOfNulls(0)
      val var10002: MutableComponent = Component.translatable("voidrix.auth.select.devicecode.reopenLink", Arrays.copyOf(`args$iv`, `args$iv`.length))
      ...
      label.setAutoTextSupplier(
         { 
            ...
            TextKt.getLiteral(`this$0`.t("voidrix.auth.select.devicecode.reopenLink")).withStyle(var10000) as Component
         }
      )
   ...
   private fun switchTo(acc: Credentials) {
      this.runWithToast(
         this.t("voidrix.auth.select.toast.switching"), acc.getUsername(), acc.getId(), this.t("voidrix.auth.select.toast.switched"), { it: Credentials ->
            it.getUsername()
         }, this.t("voidrix.auth.select.toast.switchFailed"), { 
   ...
   private fun refreshActive(acc: Credentials) {
      this.runWithToast(
         this.t("voidrix.auth.select.toast.refreshing"), acc.getUsername(), acc.getId(), this.t("voidrix.auth.select.toast.refreshed"), { it: Credentials ->
            it.getUsername()
         }, this.t("voidrix.auth.select.toast.refreshFailed"), { 
      ...
      if (this.refreshingUuid == null) {
         ...
         CompletableFuture.runAsync({ 
            ...
            if (e === SessionStatus.VALID) {
               `this$0`.rebuildOnMain({ 
                  LoadingToastHandle.success$default(`$toast`, `this$0`.t("voidrix.auth.select.toast.valid"), null, 2, null)
                  `this$0`.refreshingUuid = null
                  Unit.INSTANCE
               })
            } else if (`$acc`.getRefreshToken().length() == 0) {
               `this$0`.rebuildOnMain({ 
                  `$toast`.error(`this$0`.t("voidrix.auth.select.toast.expired"), `this$0`.t("voidrix.auth.select.toast.expiredDesc"))
                  `this$0`.refreshingUuid = null
                  Unit.INSTANCE
               })
            } else {
               try {
                  ...
                  `this$0`.rebuildOnMain({ 
                     LoadingToastHandle.success$default(`$toast`, `this$0`.t("voidrix.auth.select.toast.restored"), null, 2, null)
                     `this$0`.refreshingUuid = null
                     Unit.INSTANCE
                  })
               } catch (var6: Exception) {
                  `this$0`.rebuildOnMain({ 
                     `$toast`.error(`this$0`.t("voidrix.auth.select.toast.refreshFailed"), `$e`.getMessage())
                     `this$0`.refreshingUuid = null
                     Unit.INSTANCE
                  })
               }
   ...
   private fun updateAddState(
      transform: (gg.voidrix.client.v2.auth.AccountSelectContent.AddState.Active) -> gg.voidrix.client.v2.auth.AccountSelectContent.AddState.Active
   ) {
      if (this.addState is AccountSelectContent.AddState.Active) {
   ...
   private sealed class AddState protected constructor() {
      public data class Active(challenge: Challenge? = null, phase: String? = null, codeCopied: Boolean = false, error: String? = null) : AccountSelectContent.AddState() {
         ...
         ): gg.voidrix.client.v2.auth.AccountSelectContent.AddState.Active {
