...
import gg.voidrix.compat.http.ApiErrorTranslationKt
import gg.voidrix.compat.icon.VoidrixPlayerCache
import gg.voidrix.compat.text.TextKt
import gg.voidrix.cosmetics.v2.sync.VoidrixCustomIconService
import gg.voidrix.cosmetics.v2.ui.WardrobeCache
import gg.voidrix.networking.model.core.VoidrixUserMinimal
import gg.voidrix.networking.model.cosmetics.CosmeticUser
import gg.voidrix.networking.model.cosmetics.CustomIcon
import gg.voidrix.networking.model.cosmetics.CustomIconInfo
import gg.voidrix.networking.model.cosmetics.SupportACreatorCode
import gg.voidrix.networking.model.cosmetics.api.WardrobeResponse
...
import gg.voidrix.owolib.owo.ui.core.Size
...
public class IconSelectorComponent : FlowLayout(Sizing.Companion.fill(100), Sizing.Companion.content(), Algorithm.VERTICAL) {
   ...
   public fun build() {
      ...
      val playerUuid: UUID = VoidrixAuth.INSTANCE.getPlayerUuid()
      if (playerUuid == null) {
         this.child(this.emptyStateLabel("voidrix.icon.label.not_logged_in") as UIComponent)
      } else {
         ...
         var isVoidrixPlus: Boolean
         run label94@{
            ownedItems = var37
            val user: VoidrixUserMinimal = VoidrixPlayerCache.INSTANCE.getCached(playerUuid)
            isVoidrixPlus = user != null && user.isVoidrixPlus()
            if (user != null) {
         ...
         if ((var10 as java.util.List).isEmpty()) {
            this.child(this.emptyStateLabel("voidrix.icon.label.no_icons") as UIComponent)
         } else {
            ...
            for (var28 in CollectionsKt.chunked(flatItems, this.perRow)) {
               ...
               for (`element$ivx` in var29) {
                  row.child(this.buildIconEntry(`element$ivx` as CustomIcon, isVoidrixPlus, creatorCode, playerUuid) as UIComponent)
               }
   ...
   private fun buildIconEntry(icon: CustomIcon, isVoidrixPlus: Boolean, creatorCode: String?, playerUuid: UUID): FlowLayout {
      val size: Int = this.tileSize
      val iconSize: Int = this.tileSize - 6
      val var10000: Identifier = Identifier.fromNamespaceAndPath(
         "voidrix", this.getIconPath(icon, if (icon == CustomIconInfo.Companion.getCREATOR_CODE()) creatorCode else null)
      )
      val var18: TextureComponent = UIComponents.texture(var10000, 0, if (isVoidrixPlus) iconSize else 0, iconSize, iconSize, iconSize, iconSize * 2)
      val var19: Function0 = { 
         var var4: UUID
         run label31@{
            val var10000: VoidrixUserMinimal = VoidrixPlayerCache.INSTANCE.getCached(`$playerUuid`)
            if (var10000 != null) {
      ...
      val labelScale: VoidrixLabelButton = VoidrixLabelButton(" ", lambda_8@{ button: VoidrixLabelButton, var3: Double, var5: Double, mouse: Int ->
         if (mouse != 0) {
   ...
   private fun selectIcon(icon: CustomIcon) {
      VoidrixCustomIconService.INSTANCE
         .selectIcon(
            icon,
            { 
               var var10000: Builder = Builder.success$default(Builder(null, null, 3, null), false, 1, null)
               var `args$iv`: Array<Any> = arrayOfNulls(0)
               var var10001: MutableComponent = Component.translatable("voidrix.icon.toast.title", Arrays.copyOf(`args$iv`, `args$iv`.length))
               var10000 = var10000.title(TextKt.toSmallCapsText(var10001 as Component) as Component)
               `args$iv` = arrayOfNulls(0)
               var10001 = Component.translatable("voidrix.icon.toast.updated", Arrays.copyOf(`args$iv`, `args$iv`.length))
            ...
            { it: java.lang.Throwable ->
               ...
               val `args$iv`: Array<Any> = arrayOfNulls(0)
               val var10001: MutableComponent = Component.translatable("voidrix.icon.toast.title", Arrays.copyOf(`args$iv`, `args$iv`.length))
               var10000.title(TextKt.toSmallCapsText(var10001 as Component) as Component)
                  .description(ApiErrorTranslationKt.toTranslatedErrorText(it, "voidrix.icon.toast.error", false) as Component)
