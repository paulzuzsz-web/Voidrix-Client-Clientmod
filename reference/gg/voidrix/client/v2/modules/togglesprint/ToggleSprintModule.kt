package gg.voidrix.client.v2.modules.togglesprint
...
import gg.voidrix.compat.event.MouseScrollEventData
import gg.voidrix.compat.input.InputModifiers
import gg.voidrix.compat.mixin.input.KeyBindingPressedAccessor
import gg.voidrix.compat.mixin.input.KeyMappingAccessor
...
public object ToggleSprintModule : SingleTextHud("ToggleSprint", "{status}", null, null, false, false, false, 124) {
   ...
   public open fun onDisable() {
      ...
      try {
         ...
         (var4 as KeyBindingPressedAccessor).voidrix$setPressed(false)
      } catch (var2: Exception) {
      }
   }
   ...
   private fun sprintKeyMatchesMouseButton(button: Int): Boolean {
      ...
      val key: Key = (var10000 as KeyMappingAccessor).voidrix$getKey()
   ...
   private fun toggleSprintFromUserInput(source: String) {
      ...
      val var10000: KeyMapping = this.getSprintKey()
      (var10000 as KeyBindingPressedAccessor).voidrix$setPressed(isSprintToggled)
      MCLoggerKt.voidrixDebugLog(debugLogger, "togglesprint", "$source toggle: isSprintToggled=${isSprintToggled}")
   }

   private fun pressSprintKey() {
      if (this.isEnabled() && isSprintToggled) {
         val var10000: KeyMapping = this.getSprintKey()
         (var10000 as KeyBindingPressedAccessor).voidrix$setPressed(true)
         MCLoggerKt.voidrixDebugLog(debugLogger, "togglesprint", "pressSprintKey: forced true")
      }
   }
   ...
   private fun updateSprintKeyCache() {
      ...
      val key: Key = (sprintKey as KeyMappingAccessor).voidrix$getKey()
...
var var3: Boolean
   ...
   fun {
      ...
      MouseEvents.INSTANCE
         ...
         MCLoggerKt.voidrixDebugLog(debugLogger, "togglesprint", "joinEvent fired, enabled=${INSTANCE.isEnabled()}")
         if (INSTANCE.isEnabled()) {
      ...
      ClientEvents.INSTANCE.getRespawnEvent().listen({ it: Unit ->
         MCLoggerKt.voidrixDebugLog(debugLogger, "togglesprint", "respawnEvent fired, enabled=${INSTANCE.isEnabled()}")
         if (INSTANCE.isEnabled()) {
            repressTicksRemaining = 10
            INSTANCE.pressSprintKey()
         }
      ...
      ClientEvents.INSTANCE.getDeathEvent().listen({ it: Unit ->
         MCLoggerKt.voidrixDebugLog(debugLogger, "togglesprint", "deathEvent fired, enabled=${INSTANCE.isEnabled()}")
         Unit.INSTANCE
      })
      ClientEvents.INSTANCE.getClientTickEvent().listen(lambda_14@{ it: Unit ->
         ...
         if (INSTANCE.isEnabled() && isSprintToggled) {
            ...
            if (var10000.gui.screen() == null) {
               ...
               if (!(var4 as KeyBindingPressedAccessor).voidrix$isPressed()) {
                  MCLoggerKt.voidrixDebugLog(debugLogger, "togglesprint", "continuous enforcement: restoring sprint key")
                  INSTANCE.pressSprintKey()
               }
            }
         }
         ...
         if (INSTANCE.isEnabled() && repressTicksRemaining > 0) {
            ...
            if (repressTicksRemaining == 0) {
               MCLoggerKt.voidrixDebugLog(debugLogger, "togglesprint", "repress burst complete")
            }
   ...
   public interface StickKeyBindingExt {
      public var isVoidrixPressed: Boolean
         internal final set
   }
}
