...
public object SaturationModule : ThirdPartyModule(
   ...
   public override fun setupReflectionHooks() {
      var `$this$voidrixDebug$iv`: Logger = this.getLogger().getValue() as Logger
      if (MCLogger.IS_DEBUG) {
         `$this$voidrixDebug$iv`.info("[SaturationModule] Setting up AppleSkin reflection hooks...")
      }
      ...
      `$this$voidrixDebug$iv` = this.getLogger().getValue() as Logger
      if (MCLogger.IS_DEBUG) {
         `$this$voidrixDebug$iv`.info("[SaturationModule] AppleSkin reflection hooks setup complete.")
      }
   }
}
