...
import gg.voidrix.client.v2.modules.reauth.SessionReauth
...
public object AutoReconnect : Module("Auto Reconnect", ModuleCategory.VISUAL, true, true, false, 16) {
   ...
   public fun onDisconnectedScreenInit(reasonText: String? = null) {
      ...
      if (lastServerIp != null) {
         val `$this$voidrixDebug$iv`: Logger = this.log
   ...
   private fun cancelReconnect() {
      if (isReconnecting) {
         val `$this$voidrixDebug$iv`: Logger = this.log
         if (MCLogger.IS_DEBUG) {
            `$this$voidrixDebug$iv`.info("[AutoReconnect] Reconnect cancelled")
         }
      }
   ...
   private fun performReconnect() {
      if (needsReauth) {
         ...
         val `$this$voidrixDebug$iv`: Logger = this.log
         if (MCLogger.IS_DEBUG) {
            `$this$voidrixDebug$iv`.info("[AutoReconnect] needsReauth → SessionReauth.triggerReauth()")
         }
   ...
   public fun reconnectNow() {
      if (lastServerIp != null) {
         ...
         val `$this$voidrixDebug$iv`: java.lang.String = "[AutoReconnect] Reconnecting to $ip (attempt #${reconnectAttempts})"
         if (MCLogger.IS_DEBUG) {
            var7.info(`$this$voidrixDebug$iv`)
         }
   ...
   fun {
      ...
      ClientEvents.INSTANCE.getJoinEvent().listen({ it: Unit ->
         ...
         if (ip != null) {
            lastServerIp = ip
            val `$this$voidrixDebug$iv`: Logger = INSTANCE.log
