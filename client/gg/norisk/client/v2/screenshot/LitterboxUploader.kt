...
public object LitterboxUploader {
      ...
      try {
         ...
         if (var10000 as HttpURLConnection is HttpsURLConnection) {
            ...
            if (var43 != null) {
               ...
               val `$i$f$nrcDebug`: Logger = logger.getValue() as Logger
               if (MCLogger.IS_DEBUG) {
                  `$i$f$nrcDebug`.info("Applied TLSv1.2 SSLSocketFactory")
               }
            }
         }
   ...
   public fun uploadFile(file: File, time: String = "24h"): String? {
      val `$this$nrcDebug$iv`: Logger = logger.getValue() as Logger
