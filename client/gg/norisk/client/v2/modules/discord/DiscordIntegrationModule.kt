...
import gg.norisk.compat.event.ScreenChangeEvent
import gg.norisk.compat.host.HostedWorldService
import gg.norisk.compat.manager.ManagerProvider
import gg.norisk.compat.server.ServerInfoExt
...
public object DiscordIntegrationModule : Module("Discord Integration", ModuleCategory.VISUAL, true, true, false, 16) {
   ...
   private fun resolveScreenState(screen: Any?): String? {
      if (screen == null) {
         ...
         return if (screen.getClass().getName() == "gg.norisk.minigames.base.MinigameScreen")
            "Playing a Minigame"
            else
            (
               if (screen.getClass().getName() == "gg.norisk.cosmetics.v2.ui.newwardrobe.NewWardrobeScreen")
                  "Browsing Wardrobe"
                  else
                  (
                     if (screen.getClass().getName() == "gg.norisk.mcreal.ui.McRealScreen")
                        "Browsing McReal"
                        else
                        (
   ...
   private fun formatServerAddress(ip: String): String {
      ...
      if (!var8 && ((serverData as? ServerInfoExt) == null || !(serverData as? ServerInfoExt).getNrc_friendsVisible())) {
