...
import gg.norisk.compat.chat.AddVisibleMessageEvent
import gg.norisk.compat.chat.BreakChatLinesEvent
import gg.norisk.compat.chat.ChatEvents
import gg.norisk.compat.chat.SeenPlayersCache
import gg.norisk.compat.chat.VisitFormattedData
import gg.norisk.compat.chat.AddVisibleMessageEvent.Injection
import gg.norisk.compat.chat.SeenPlayersCache.MinecraftPlayer
...
public object ChatHeads : Module("Chat Heads", ModuleCategory.VISUAL, false, true, false, 20) {
   ...
   public fun setCurrentSkinTextures(line: Line) {
      ...
      val var10000: Supplier = (line as ChatHudLineVisibleExt).nrc_skinTextures
      currentSkinTextures = if (var10000 != null) var10000.get() as PlayerSkin else null
      val uuid: UUID = (line as ChatHudLineVisibleExt).nrc_messageSender
      if (currentSkinTextures == null && (line as ChatHudLineVisibleExt).nrc_tellReceiver != null) {
      ...
      ChatEvents.INSTANCE.getVisitFormattedEvent().listen(lambda_1@{ data: VisitFormattedData ->
         if (!INSTANCE.isEnabled()) {
            ...
            if (currentMessage != null && startHeadApply) {
               var `$this$nrcDebug$iv`: Logger
               var var14: java.lang.String
               run label111@{
                  `$this$nrcDebug$iv` = INSTANCE.getLogger().getValue() as Logger
               ...
               when (ChatHeads.WhenMappings.$EnumSwitchMapping$0[INSTANCE.mode.ordinal()]) {
                  ...
                  3 -> {
                     ...
                     val `$this$nrcDebug$ivx`: Logger = INSTANCE.getLogger().getValue() as Logger
                     val `message$ivx`: java.lang.String = "BEFORE_NAME -> $var8"
                     if (MCLogger.IS_DEBUG) {
                        `$this$nrcDebug$ivx`.info(`message$ivx`)
                     }
