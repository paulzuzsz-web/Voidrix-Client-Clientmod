...
import gg.norisk.compat.event.CrosshairRenderEvent
...
import gg.norisk.ui.api.value.ColorAttributeValue
...
public object CustomCrosshair : Module("Custom Crosshair", ModuleCategory.VISUAL, false, false, false, 28) {
   ...
   public final var activeGrid: NrcPixelGridDrawComponent?
      internal set

   private final var cachedRects: List<gg.norisk.client.v2.modules.crosshair.CustomCrosshair.PixelRect>?
   ...
   private fun decomposePixelsToRects(pixels: Map<Pair<Int, Int>, Int>): List<gg.norisk.client.v2.modules.crosshair.CustomCrosshair.PixelRect> {
      if (pixels.isEmpty()) {
   ...
   private fun getDecomposedRects(): List<gg.norisk.client.v2.modules.crosshair.CustomCrosshair.PixelRect> {
   ...
   private fun getScaleDiff(): Double {
      ...
      if ((var6 as IWindowScaleExt).getNrc_scaledResolution() == null) {
         return 1.0
      } else {
         var10000 = Minecraft.getInstance()
         val customScale: Double = var10000.getWindow().getGuiScale()
         return if (customScale > 0.0) ext.getNrc_mcScaleFactor() / customScale else 1.0
      }
   }
   ...
   private data class PixelRect(x: Int, y: Int, w: Int, h: Int, color: Int) {
      ...
      public fun copy(x: Int = this.x, y: Int = this.y, w: Int = this.w, h: Int = this.h, color: Int = this.color): gg.norisk.client.v2.modules.crosshair.CustomCrosshair.PixelRect {
