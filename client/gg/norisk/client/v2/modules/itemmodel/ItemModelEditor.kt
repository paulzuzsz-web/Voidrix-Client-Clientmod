...
import gg.norisk.compat.client.ItemSearch
import gg.norisk.compat.client.MCRegistryKt
import gg.norisk.compat.client.ItemSearch.ItemSearchEntry
...
public object ItemModelEditor {
   ...
   private fun buildResetButton(itemName: String, onReset: () -> Unit): LabelComponent {
      ...
      val var10001: MutableComponent = Component.translatable("nrc.itemmodel.reset.tooltip", Arrays.copyOf(var8, var8.length))
   ...
   public fun build(settingsPanel: FlowLayout, module: ItemModel) {
      val searchField: NrcInputField = NrcInputField(120, "", 0.75F, true)
   ...
   private fun buildItemEditor(container: NrcCollapsibleContainer, module: ItemModel, itemId: Identifier, rebuild: () -> Unit) {
      ...
      val previewCheckbox: NrcCheckbox = NrcCheckbox(0.75F)
      ...
      groundTransform.child(
         TransformEditorComponent("Ground", itemTransformation.getOrCreate(NrcItemDisplayContext.GROUND).scale, 0.0, 4.0, 0.01, 2, 1.0F, true, { 
      ...
      groundTransform.child(
         TransformEditorComponent(
            "1st Person", itemTransformation.getOrCreate(NrcItemDisplayContext.FIRST_PERSON_RIGHT_HAND).scale, 0.0, 4.0, 0.01, 2, 1.0F, true, { 
               `$module`.saveTransformations()
               Unit.INSTANCE
            }
         ) as UIComponent
      )
      groundTransform.child(TransformEditorComponent("GUI", itemTransformation.getOrCreate(NrcItemDisplayContext.GUI).scale, 0.0, 4.0, 0.01, 2, 1.0F, true, { 
         `$module`.saveTransformations()
         Unit.INSTANCE
      }) as UIComponent)
      content.child(groundTransform as UIComponent)
      val contextSwitch: Array<Any> = arrayOfNulls(0)
      val var10000: MutableComponent = Component.translatable("nrc.itemmodel.label.advanced", Arrays.copyOf(contextSwitch, contextSwitch.length))
      val advancedCollapsible: <unrepresentable> = object : NrcCollapsibleContainer {
      ...
      var28.title("Context")
      val var31: V3EnumSwitcher = V3EnumSwitcher(NrcItemDisplayContext.Companion.validEntries(), module.currentMode, { it: NrcItemDisplayContext ->
         it.getDisplayName()
      }, 80)
      var31.onChanged().subscribe({ ctx: NrcItemDisplayContext ->
   ...
   @JvmStatic
   fun `build$rebuildItems`(listScroll: ScrollContainer<FlowLayout>, listChild: FlowLayout, searchField: NrcInputField, `$module`: final ItemModel) {
      ...
      for (`element$iv` in ItemSearch.search$default(ItemSearch.INSTANCE, ItemSearch.INSTANCE.buildIndex(), searchField.text(), 0, 4, null)) {
         ...
         try {
            ...
            val collapsible: <unrepresentable> = object : NrcCollapsibleContainer {
               public open fun toggleExpansion() {
                  super.toggleExpansion()
                  if (this.expanded()) {
